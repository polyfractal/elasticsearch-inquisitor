...
|_. elasticsearch-inquisitor branch |_. ElasticSearch |
| master | 1.0+ |
| 0.90.x | 0.90.x |
...
* If you need support less than 0.90.x, version 0.1.2 _should_ work. Inquisitor requires ElasticSearch 0.19.0.RC1 for the 'Analyzers' and 'Tokenizers' tabs, but the main 'Queries' tab will function with versions >= 0.14.0.  No effort will be taken to support anything less than 0.90.x

h3. Installation

To install Inquisitor onto your node, run <code>bin/plugin install polyfractal/elasticsearch-inquisitor</code>

Inquisitor automatically connects to <code>localhost</code>, so it must be run on a node that is web-accessible.  To access Inquisitor, open <code>http://localhost:9200/_plugin/elasticsearch-inquisitor/</code>
